...
const Footer: React.FC = () => {
  ...
  return (
    <footer
      className="relative bg-gradient-to-br from-amber-50/80 via-amber-100/70 to-amber-200/50 border-t border-amber-300/50 overflow-hidden"
      style={{ contentVisibility: "auto", containIntrinsicSize: "680px" }}
    >
      <div className="pointer-events-none absolute top-24 right-0 w-40 h-40 rounded-full bg-amber-300/25 blur-3xl" />
      <div className="pointer-events-none absolute bottom-10 left-10 w-28 h-28 rounded-full bg-amber-200/25 blur-3xl" />

      <div className="container mx-auto px-4 py-16 relative z-10 text-stone-900">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          >
            <h3 className="text-3xl font-bold mb-6 select-none cursor-default">
              RK <span className="text-amber-700 font-extrabold">Store</span>
            </h3>
            ...
            <ul className="space-y-3">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    to={link.to}
                    className="text-stone-800 hover:text-amber-700 transition-colors flex items-start group"
                  >
                    <span className="w-2 h-2 bg-amber-600 rounded-full mt-2 mr-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          >
            ...
            <ul className="space-y-3 max-h-48 overflow-y-auto">
              {loadingCats ? (
                <>
                  <li className="h-4 w-40 bg-amber-200 rounded animate-pulse" />
                  <li className="h-4 w-32 bg-amber-200 rounded animate-pulse" />
                  <li className="h-4 w-24 bg-amber-200 rounded animate-pulse" />
                </>
              ) : categories.length > 0 ? (
                ...
                <li className="text-stone-500 text-sm select-none">No categories available.</li>
              )}
            </ul>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-5">
              <div className="flex items-center space-x-4">
                ...
                <a href={`mailto:${email}`} className="text-stone-800 hover:text-amber-700 transition-colors">
                  {email}
                </a>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <a href={`tel:${phone.replace(/\s/g, "")}`} className="text-stone-800 hover:text-amber-700 transition-colors">
                  {phone}
                </a>
              </div>
            </div>
        ...
        <motion.div
          className="border-t border-amber-300 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center text-stone-600"
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.2 }}
